...
def copy_to_new_good_exons_dir(df) -> None:
    ...
    '''
    dir_path = os.path.pardir(args.path_to_stats_table)
    for subdir in os.listdir(dir_path):
        ...
        for entry in df.iterrows():
            ...
            command = f"cp {dir_path}/{exon_name} {new_good_exons_dir}"
            print(command)
            # os.system(command)

        # making the stats_table for the selected exons
        sub_stats_table_path = f"{new_good_exons_dir}/stats_table.csv"
        print("writing stats to", sub_stats_table_path, index=True, header=True, line_terminator='\n', sep=";")
        df.to_csv(sub_stats_table_path)
