...
def prRed(skk): print("Cell\033[91m {}\033[00m" .format(skk))
# def prRed(s): pass

class CgpHmmCell(tf.keras.layers.Layer):
    def __init__(self):
        super(CgpHmmCell, self).__init__()
        self.state_size = [5,1,1] # todo is this 5 hidden alphas an the loglike?, i dont think so
        self.state_size_without_index_of_current_cell = self.state_size[0]  # i want to know the the i of alpha(q,i) in the return sequences
        self.alphabet_size = 4


    def build(self, input_shape):
        prRed("build of CgpHmmCell")
        self.transition_kernel = self.add_weight(shape = (self.state_size_without_index_of_current_cell, self.state_size_without_index_of_current_cell),
                                              initializer="random_normal",
                                              trainable=True)
        self.emission_kernel = self.add_weight(shape = (self.state_size_without_index_of_current_cell, self.alphabet_size),
                                              initializer="random_normal",
                                              trainable=True)
    @property
    def A(self):
